...
class PracticeService:
    ...
    def filtered(self, track_id: str, difficulty: str) -> list[Exercise]:
        ...
        if track_id != "all":
            results = [exercise for exercise in results if exercise.track_id == track_id]
        if difficulty != "all":
            results = [exercise for exercise in results if exercise.difficulty == difficulty]
        return results
        ...
        try:
            ...
            try:
                ...
                if mode == "query":
                    ...
                    expected = self._normalize_rows(expected_rows, fixture.get("ordered", False))
                    if actual == expected:
                        score += 70
                    ...
                    actual = self._normalize_rows(rows, fixture.get("ordered", False))
                    expected = self._normalize_rows(fixture.get("expected_rows", []), fixture.get("ordered", False))
                    if actual == expected:
                        score += 70
    ...
    def _evaluate_sql(self, exercise: Exercise, code: str) -> EvaluationResult:
        ...
        if exercise.id in {
            ...
            if exercise.id == "db-orders-foreign-key":
                ...
                return self._evaluate_sql_fixture(exercise, code, SQL_QUERY_FIXTURES["db-explain-users-query"])
